...
It retrieves the top `n` stories from the Hacker News API and returns them ordered by score, from highest to lowest.
...
If n is invalid (<= 0 or too large), the API returns 400 Bad Request.
...
Used batching plus Task.WhenAll to balance performance and external API load.
...
Introduce a distributed cache such as Redis for scalability across multiple instances.
Implement retry policies, for example Polly, for transient HTTP failures.
